...
import com.newgame.models.SubAgentsInfo;
...
public class MainFragment extends Fragment {
    ...
    private ArrayList<SubAgentsInfo> subAgentsInfoArrayList = null;
    ...
    private Spinner facilitySpinner, subAgentNameSpinner;
    ...
    private SubAgentsInfo selectedSubAgent;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        subAgentNameSpinner = (Spinner) rootView.findViewById(R.id.subAgentNameSpin);
...
//		openRadio.setChecked(true);
        ...
        amountDetails = recordInsertDB.getOperations(Queries.getInstance().getSumDetails(CommonUtils.selectedGame, agentId, game_id));
    ...
    public void getGameTimingsDetails() {
        ...
        subAgentNameSpinner.setOnItemSelectedListener(new OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                 selectedSubAgent = subAgentsInfoArrayList.get(position);
            }
    ...
    private void updateSubAgents() {
        GamesLiveData.getGenericData(new ApplicationThread.OnComplete<String>(ApplicationThread.OnComplete.UI) {
            ...
            public void run() {
                ...
                try {
                    android.util.Log.i("Response MEssage...", result.toString());
                    Type collectionType = new TypeToken<Collection<SubAgentsInfo>>() {
                    }.getType();
                    Gson googleJson = new Gson();
                    subAgentsInfoArrayList = googleJson.fromJson((String) result, collectionType);
                    android.util.Log.i("", "Agent assign.." + subAgentsInfoArrayList);
                    subAgents = new String[subAgentsInfoArrayList.size()];
                    for (int i = 0; i < subAgentsInfoArrayList.size(); i++) {
                        subAgents[i] = subAgentsInfoArrayList.get(i).getSubAgentName();
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                } finally {
                    progressDialog.dismiss();
                    if (subAgents != null) {
                        subAgentNameSpinner.setAdapter(new CommonArrayAdapter(getActivity(), subAgents));
                    }
                }
            }
        }, String.format(Config.live_url + Config.SubAgentinfo, agentId));
    }
    ...
    public void getGameStatus() {
        ...
        GamesLiveData.getGameDetails(new ApplicationThread.OnComplete<String>(ApplicationThread.OnComplete.UI) {
            ...
            public void run() {
                ...
                try {
                    Type collectionType = new TypeToken<Collection<SubGameStatus>>() {
                    }.getType();
                    Gson googleJson = new Gson();
...
//	}
    ...
    public JSONArray createGroupInServer(Activity activity, ArrayList<GameTransactionsModel> groups) {
        ...
        try {
            ...
            for (int i = 0; i < groups.size(); i++) {
			    ...
			    jGroup.put("SubAgentID", groups.get(i).getSubAgentID());
    ...
    public void addRecordToDb(String number, String amount) {
        ...
        eachEntryDataMap.put("SubAgentID", selectedSubAgent.getSubAgentId());
